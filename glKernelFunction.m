function [L] = glKernelFunction(w,T)
...
for wprime = w
    i=i+1;
    L(i,:) = hcotangent + log(double(gamma(sym(1/2+1i*(w-wprime*ones(size(w)))/denominator)))) - ... 
        log(double(gamma(sym(1/2-1i*(w+wprime*ones(size(w)))/denominator))));
