function [dielec,Pi] = FittoRealDielec(x,w,kernel)
...
if length(x) == 7
        Pi = @(wprime) (wprime>=x(1))*x(2) + (wprime>=x(3))*(-x(2)+x(4)) ...
                        + (wprime>=x(5))*(x(6)-x(4)) - (wprime>=x(7))*x(6); %define 3 blocks of histogramic representatio
elseif length(x) == 5
                Pi = @(wprime) (wprime>=x(1))*x(2) + (wprime>=x(3))*(-x(2)+x(4)) ...
                        + (wprime>=x(5))*(-x(4));
elseif length(x) == 3
...
dielec = real(transpose(CondtoDielec(cond,w)));
