function [Chi,chi0] = RPAChi(frequency,qx,qy)
...
KpereV = 11604.505;
beta = 1/(5/KpereV); %1/eV
Gamma = 0.001; %eV (1 meV)
        ...
        for qyi = qy
            ...
            for kxi = kx
                for kyi = ky
                    ...
                    fermiEKQ = 1/(exp(beta*EKQ)+1);
                    
                    fermiEK = 1/(exp(beta*EK)+1);
                    ...
                    sum =   sum - CplusQK*(fermiEKQ-fermiEK)/(w+1i*Gamma+EKQ-EK) + ...
                        CminusQK*(fermiEKQ-fermiMinusEK)/(w+1i*Gamma-EKQ-EK) + ...
                        CminusQK*(fermiMinusEKQ-fermiEK)/(w+1i*Gamma+EKQ+EK) - ...
                        CplusQK*(fermiMinusEKQ-fermiMinusEK)/(w+1i*Gamma-EKQ+EK);
